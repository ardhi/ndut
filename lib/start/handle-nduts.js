...
module.exports = async function (config) {
  const { _, fs, defNdutKeys, aneka } = this.ndut.helper
  const { getModuleDirDeep, requireBase } = aneka
  let appModes = []
  ...
  for (const n of config.nduts) {
    ...
    let cfg = { name: n }
    const file = `${dir}/ndut/config.json`
    if (fs.existsSync(file)) {
      cfg = await requireBase(file)
    ...
    if (cfg.name === 'app') {
      ...
      cfg.alias = cfg.alias || (n.slice(0, 5) === 'ndut-' ? n.slice(5) : n) // fix. can't be overriden
      cfg.prefix = cfg.prefix || (n.slice(0, 5) === 'ndut-' ? n.slice(5) : n) // can be overridden via config.json
    }
  ...
  for (const n of config.nduts) {
    ...
    if (cfg.dependency.length > 0 && _.intersection(config.nduts, cfg.dependency).length === 0)
      throw new Error(`Dependency for '${name}' unfulfilled: ${humanJoin(cfg.dependency)}`)
  }
