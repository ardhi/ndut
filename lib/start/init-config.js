...
const getConfig = async () => {
  ...
  if (!appModes.includes(cfg.appMode)) throw new Error(`Invalid app mode '${cfg.appMode}'. Supported modes: ${humanJoin(appModes)}`)
  cfg.debug = process.env.DEBUG || argv.debug
  if (cfg.appMode === 'build') cfg.debug = true
...
module.exports = async function () {
  ...
  fastify.log.info(`Entering '${config.appMode}' mode`)
